...
func RunIntegrationPlugin(ctx context.Context, cfg IntegrationHostConfig, wasmPath string, stdin []byte, maxStdout int) ([]byte, error) {
	if maxStdout <= 0 {
...
func writeResp(mem api.Memory, respPtr, respCap uint32, jsonOut []byte) int32 {
	if uint32(len(jsonOut)) > respCap {
		return ogErrRespTooBig
	}
	if !mem.Write(respPtr, jsonOut) {
		return ogErrBadMemory
	}
	return int32(len(jsonOut))
}
