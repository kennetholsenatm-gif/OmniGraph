...
func (p *IPMIProvider) sendCommand(ctx context.Context, target *Target, cmd IPMICommand) (*IPMIResponse, error) {
	...
	case IPMICommandGetChassisStatus:
		return &IPMIResponse{
			CompletionCode: IPMICompletionCodeNormal,
			Data: []byte{
				0x01, // Power on
	...
	case IPMICommandChassisControl:
		return &IPMIResponse{
			CompletionCode: IPMICompletionCodeNormal,
			Data:           []byte{},
		}, nil
		
	case IPMICommandGetBootOptions:
		return &IPMIResponse{
			CompletionCode: IPMICompletionCodeNormal,
			Data: []byte{
	...
	case IPMICommandSetBootOptions:
		return &IPMIResponse{
			CompletionCode: IPMICompletionCodeNormal,
			Data:           []byte{},
		}, nil
	...
	default:
		return &IPMIResponse{
			...
			Data:           []byte{},
		}, fmt.Errorf("unsupported IPMI command: 0x%02X", cmd.Command)
	}
}
