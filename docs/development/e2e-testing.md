...
- **Mock IR, graph, and state fixtures** — versioned JSON and workspace layouts checked into `e2e/fixtures/` (or similar), so scenarios are **reproducible**.
- **Drivers** — Go tests that call **`pkg/emitter`** (and related packages) directly against fixtures and assert outputs—no subprocess of a multi-command tool.
...
The exact layout follows the harness implementation; the **contract** for contributors is: **if it spans Emitter Engine + external tool boundaries and you care about failure modes, it belongs in E2E**.
...
| **`e2e/`** | **Full pipeline**: optional Wasm build, mocked Ansible/API behavior, **injected failures** (timeouts, 500s, malformed bodies, non-zero playbook exits). |
...
E2E scenarios **must** include **unhappy paths**: slow responses, truncated JSON, exit code 2 from a fake playbook runner, missing inventory keys. The goal is not sadism—it is **confidence** that operators see **actionable errors** and that the **browser** and **control plane** remain usable when one leg of the pipeline misbehaves.
