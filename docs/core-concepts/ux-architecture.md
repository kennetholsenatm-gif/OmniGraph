...
**Gestalt on the canvas:** the graph uses **proximity** (related resources cluster visually) and **similarity** (consistent styling for the same resource kinds) so operators can **scan** for blast radius and correlated failure without reading prose first. That visual grouping is intentional infrastructure UX—not decorative layout.
