...
2. From repo root: `go run ./cmd/omnigraph --web-dist packages/web/dist` (or run a built binary with the same flags—see [CI and contributor automation](ci-and-contributor-automation.md))

Then open the URL the process prints (loopback by default). **Inventory → Load from OmniGraph server** can fill summary data when `/api/v1/workspace/summary` is available. Run the binary with `-h` for authentication and experimental endpoint flags.
