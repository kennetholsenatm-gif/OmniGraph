...
- **Architecture refactor (merged via PR #24):** React app under `packages/web`; Reconciler
  Engine in `pkg/reconciler` (public); root `go.work` for Wasm modules; HCL Wasm handlers
  ...
  [Reconciler Engine](../core-concepts/reconciler-engine.md), [ADR 008](../core-concepts/adr/008-wasm-bridge-hardening.md),
