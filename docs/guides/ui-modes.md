...
| **Reconciliation** | Does reality match what we declared? | **Inventory**, **Pipeline** | Paste Terraform/OpenTofu **state** and **plan JSON**, Ansible **INI**; optional repo scan; **local workspace server** summary and **SSE** `GET /api/v1/workspace/stream` ([`InventoryTab`](../../packages/web/src/mvp/InventoryTab.tsx), [`PipelineTab`](../../packages/web/src/mvp/PipelineTab.tsx)). Reconciliation snapshot context (`omnigraph/reconciliation-snapshot/v1`) adds BOM entity/relation counts plus drift cues for triage; details in [BOM automation](../core-concepts/bom-automation.md). **Pipeline** keeps a separate schema path for execution previews; **Schema Contract** holds the manifest path for exports—use **Copy manifest path to Pipeline** when you want them to match ([merge notes](../core-concepts/inventory-sources.md#browser-workspace-how-inventory-rows-combine), [Using the web workspace](../using-the-web.md)). |
