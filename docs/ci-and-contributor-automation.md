...
Build and run the server (APIs only):
...
With a built web app (after `cd packages/web && npm run build`):
...
Experimental routes (`POST /api/v1/security/scan`, inventory, host-ops, ingest, sync WebSocket, drift) stay **off** unless you pass the matching **`--enable-*`** flags **and** `--auth-token` (or `OMNIGRAPH_SERVE_TOKEN`). Run `./bin/omnigraph -h` for the full flag list.
